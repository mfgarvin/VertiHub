...
#	print(time.localtime().tm_hour - 12, time.localtime().tm_min)
	while running == 1:
		...
		if actualTime.tm_hour - hour == 1 or actualTime.tm_hour - hour == -11 or firstrun == 1: 
		...
		if actualTime.tm_min - minute == 1 or actualTime.tm_min - minute == -59 or firstrun == 1:
			...
			if 1 <= minute <= 9:
				for x in xrange(minute):
					strip.setPixelColor(inuseHour + x + 1, minuteColor)
			if 10 <= minute:
#Issue Here -------->				
				for x in xrange(int(minute_str[0])):
					strip.setPixelColor(inuseHour + x + 3, minuteColor)
#				if int(minute_str[1]) == 0:
#					strip.setPixelColor(inuseHour + int(minute_str[0]) + 2, placeholder)
#				else:
#					for x in xrange(int(minute_str[1])):
#						strip.setPixelColor(inuseHour + int(minute_str[0]) + 1, Color(0, 0, 0))
#						strip.setPixelColor(inuseHour + int(minute_str[1]) + x, minuteColor)
		if second == 'OFF':
			#Turn LED On
			print('on')
			second = 'ON'
